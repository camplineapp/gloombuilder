...
export default function GeneratorScreen({ onClose, onSave, onRunThis }: GeneratorScreenProps) {
  ...
  const addFromQA = (name: string, si: number, isCustom: boolean) => {
    ...
    setGr(gr.map((s, i) => i !== si ? s : { ...s, exercises: [...s.exercises, add] }));
    setQaQ("");
          ...
          <div key={si} style={{ marginTop: 24 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {eL === String(si) ? (
                  <input autoFocus value={sec.label} maxLength={60} onChange={e => setGr(gr.map((s, i) => i !== si ? s : { ...s, label: e.target.value }))} onBlur={() => setEL(null)} onKeyDown={e => { if (e.key === "Enter") setEL(null); }} style={{ fontFamily: F, color: sec.color, fontSize: 13, textTransform: "uppercase", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "5px 12px", outline: "none", width: 200, fontWeight: 700 }} />
                ) : (
