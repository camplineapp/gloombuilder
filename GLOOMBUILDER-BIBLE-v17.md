# GLOOMBUILDER BIBLE v17
## Complete Product & Design Truth Document
### May 1, 2026 — V2-PIVOT FEATURE-COMPLETE EDITION

*This document supersedes Bible v16 (April 30, 2026). All v16 content is preserved verbatim. v17 adds: (1) a comprehensive **V17 SESSION RECAP** at the top capturing eleven commits across three clusters from the May 1 session; (2) the **Cluster A — quick wins** commit detailing source pill consistency, CopyModal qName fix, gear icon discoverability, and save-without-leaving-screen; (3) the **Cluster B — platform robustness** commits (4 separate) covering Android pull-to-refresh suppression, localStorage autosave across Builder/Generator/CreateExercise, PWA versioned-cache update notifier with banner, and Android hardware back button via popstate; (4) the **Cluster C — Item 5 unified Library Exercises tab** commit collapsing Codex/Community split into one feed sorted newest-first with creator-name search; (5) the **Cluster C — Item 5B exercise edit flow** commit fixing the latent bug where Q Profile exercise cards were tappable-but-broken plus three structural bugs surfaced during the diagnostic; (6) **two big strategic decisions**: (a) deferred Item 6+7 (find HIMs / follow system) — explicit decision to NOT re-introduce social-graph machinery now, revisit later; (b) deferred Item 12 (Notepad mode) — explicit decision to ship v2-pivot to main first and gather real PAX usage data before committing 4-6 weeks to the third creation path; (7) updated canonical patterns for popstate-driven hardware back unwinding, localStorage drafts envelope, PWA SW versioning via package.json injection, and the Layer 3 destructive footer extended to the exercise edit form. The `v2-pivot` branch is feature-complete after eleven commits today; production at gloombuilder.app remains stable on `main` (v1 architecture, 4-tab nav). The next session's work is the merge from `v2-pivot` to `main`, the v2.0.0 tag, and gloombuilder.app deployment.*
