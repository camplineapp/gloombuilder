# GLOOMBUILDER BIBLE v18
## Complete Product & Design Truth Document
### May 2, 2026 — V2-PIVOT NOTEPAD-COMPLETE EDITION

*This document supersedes Bible v17 (May 1, 2026). All v17 content is preserved verbatim. v18 adds: (1) a comprehensive **V18 SESSION RECAP** at the top capturing the May 2 Notepad-build session — three commits implementing **Item 12 — Notepad v0 MVP** end-to-end (Stage 3 commit `a3f8c9b`-equivalent + parser priority swap fix `225d8e0` + difficulty default fix), one Supabase migration applied to production, plus extensive design discussion that produced the canonical parser philosophy; (2) the full **Notepad v0 architecture documentation** — parser priority order, the hybrid-implicit-explicit philosophy, the position-driven section detection rule, the markers (-/*/• for notes, > for transitions), the legend placement decision (Option D — empty-textarea placeholder + help drawer), the placeholder content, the section-level note routing rule (notes OR transitions appearing under a section header before the first exercise become section qNotes), the no-data-model-changes confirmation that collapsed the Option A/B fork into a single clean implementation; (3) the **strategic re-prioritization** that brought Notepad forward into the v2-pivot scope — Ritz overrode the v17 deferral after a 6-hour design session in this chat established that the data model already supported everything Notepad needed; (4) two new canonical patterns — **the diagnostic-first / spec-second / build-third / test-or-trust-fourth rhythm explicitly applied across 3 stages** for the Notepad commit, and **the real-input-test-trumps-synthetic-test discipline** that surfaced the parser priority bug post-Stage 3; (5) the parser bug-fix loop documented in detail (Bug 1 — Q4 fired before positional check; Bug 2 — Motivator library content gap, deferred; Bug 3 — downstream of Bug 1, fixed itself); (6) the **Supabase migration** applied to production (`from_notepad boolean NOT NULL DEFAULT false` on the beatdowns table); (7) updated next-session priorities — **`v2-pivot` is now genuinely feature-complete and ready to merge** with the Notepad MVP shipped, real-tested on Vercel preview, and confirmed working by Ritz. Latest commit hash: difficulty fix from this session. Ten total commits today on `v2-pivot` building on top of v17's nine commits from yesterday. The merge to `main` + v2.0.0 tag + gloombuilder.app deployment is the next session's first action.*

---

## V18 SESSION RECAP — MAY 2, 2026 (READ THIS FIRST IF PICKING UP MID-FLIGHT)
